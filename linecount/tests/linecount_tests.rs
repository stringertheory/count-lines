...
fn estimate_mean_within_four_stddevs_of_exact() {
    ...
    for i in 0..NUM_TRIALS {
        ...
        let estimate = count_lines_estimate(path, opts).expect("failed to estimate number of lines");
        estimates.push(estimate as f64);
    }

    let mean: f64 = estimates.iter().sum::<f64>() / NUM_TRIALS as f64;
    let stddev: f64 = (estimates.iter().map(|x| (x - mean).powi(2)).sum::<f64>() / NUM_TRIALS as f64).sqrt() / (NUM_TRIALS as f64).sqrt();
    let exact_f64 = exact as f64;
    let lower = mean - 4.0 * stddev;
    let upper = mean + 4.0 * stddev;

    eprintln!("\n→ estimate mean: {:.0}, stddev: {:.2}, exact: {}, range: [{:.0}, {:.0}]", mean, stddev, exact, lower, upper);

    assert!(
        (exact_f64 >= lower) && (exact_f64 <= upper),
        "Estimate mean {} ± 4σ ({:.2}) does not include exact value {}",
        mean, stddev * 4.0, exact
    );
}
